...
parser.add_argument("--hod",help='hod mock number')
...
if args.prog == 'dark':
    ...
    if args.split_snapshot == 'y':
        zs = {'ELG':{'z0.950':[0.,1.1], 'z1.325':[1.1,99.]}, 'LRG':{'z0.500':[0.,0.6]}, 'QSO':{'z1.400':[0.,99.]}} #'z0.800':[0.6,99.]}
    else:
        zs = {'ELG':'z1.100', 'LRG':'z0.500', 'QSO':'z1.400'}
...
for real in range(args.realmin, args.realmax):
    if not (args.mockver is None):
        if args.mockver == 'ab_firstgen':
            ...
            out_file_name = os.path.join(mockdir, 'forFA{real}_{hod}.fits'.format(real=real,hod=args.hod))
            ...
            file_name = 'cutsky_{TYPE}_{Z}_AbacusSummit_base_c000_ph{PH}.fits'
            file_name = f"cutsky_LRG_z0.500_000_withrsd_{args.hod}.fits"#HOD_DEFAULT_REDUX.fits"            
            mockdir = os.path.join(args.base_output, 'SecondGenMocks', Abacus_dir)
            #if args.split_snapshot == 'y':
            print('Secondgen',file_name)
            create_dir(mockdir)
            ...
            out_file_name = os.path.join(mockdir, 'forFA{real}_{hod}_Y1.fits'.format(real=real,hod=args.hod))
            #else:
            ...
            out_file_name = os.path.join(mockdir, 'forFA{real}_Y1.fits'.format(real=real))
        ...
        #file_name = "cutsky_LRG_z0.500_000_withrsd_1.fits"
        print('filename',file_name)
        mockdir = args.base_output
        out_file_name = os.path.join(mockdir, 'forFA{0}_Y1.fits'.format(real))
        ...
        if args.mockver == 'ab_firstgen' or args.mockver == 'ab_secondgen' or args.mockver == 'ab_thirdgen':
            ...
            if args.split_snapshot == 'y':
                ...
                for bins in zs[type_]:
                    print(bins)
                    thepath = os.path.join(mockpath, bins,f'cutsky_LRG_z0.500_000_withrsd_{args.hod}.fits' )#file_name.format(TYPE = type_, Z = bins, PH = "%03d" % real))
                    ##TEMPthepath = os.path.join(mockpath, type_, bins, file_name.format(TYPE = type_, Z = bins, PH = "%03d" % real))
                    print('thepath')
                    print(thepath)
                    fitsA = fitsio.FITS(thepath)
                    column_names = fitsA[1].get_colnames()
                    print(column_names,'SPLIT SNAP??')
                ...
                thepath = os.path.join(mockpath,zs[type_],f'cutsky_LRG_z0.500_000_withrsd_{args.hod}.fits' )# file_name.format(TYPE = type_, Z = zs[type_], PH = "%03d" % real))
                print(thepath,'the path')
                thepath = os.path.join(mockpath,'z0.500',file_name)
                from astropy.io import fits
                #with fits.open(thepath) as hdul:
                 #   test=hdul[1].data
                   # print(hdul[1].columns)
                fitsA = fitsio.FITS(thepath)
                column_names = fitsA[1].get_colnames()
                print(column_names,'ELSE 1')
                data = fitsio.read(thepath, columns=['RA','DEC','Z','Z_COSMO','STATUS','ID'])#f[1].data
    ...
    print(targets.columns,'astropy cols')
    print(targets)
    del datat
    ...
    if args.apply_mask == 'y':
        ...
        mask = bitmask.get_nobsandmask(targets, nproc=128)
        maskv = mask.get_nobsandmask(nproc=128)
